...
The template lives at `/etc/fsh/config.json.template`, outside the web root, so it is never served and `config.json` is re-rendered on every start - restarting the same container is safe.
